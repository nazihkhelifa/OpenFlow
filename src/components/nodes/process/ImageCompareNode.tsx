      ...
      {imageA && imageB ? (
        <div className="flex-1 relative nodrag nopan nowheel">
          ...
          <div className="absolute top-2 right-2 flex items-center gap-1">
            <MediaExpandButton
              nodeId={id}
              mediaUrls={[imageA, imageB].filter(Boolean) as string[]}
              className="w-6 h-6 bg-black/50 hover:bg-black/70 text-white rounded flex items-center justify-center transition-colors"
            />
            <div className="bg-black/50 text-white text-[10px] font-medium px-2 py-1 rounded pointer-events-none">
              B
            </div>
          </div>
        </div>
