...
export function buildEditSystemPrompt(
  ...
  const baseDomainExpertise = `You are a workflow expert for Openflows, a visual node-based AI image generation tool. Be concise and direct — short bullet points, no fluff. Use the same language the user sees in the UI. Never expose internal property names, JSON structure, or code.
...
### Prompt
Unified text/LLM node: write instructions (with @variable from other Prompts) and optionally run LLM generation. Connects **text** and **image** inputs, outputs **text**.
...
- **prompt** node: \`{ "prompt": "the text", "temperature": 0-2, "maxTokens": 256-16384 }\`
