...
describe("workflowStore integration tests", () => {
  ...
  describe("getConnectedInputs", () => {
    describe("Basic data extraction scenarios", () => {
      ...
      it("should extract text from prompt node (outputText)", () => {
        const store = useWorkflowStore.getState();
        const testOutput = "Generated text from LLM";

        useWorkflowStore.setState({
          nodes: [
            createTestNode("prompt-1", "prompt", { outputText: testOutput, prompt: "" }),
  ...
  describe("validateWorkflow", () => {
    ...
    describe("annotation node validation", () => {
      it("should return error when no image connected and no sourceImage", () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.errors).toContain('Layer Editor node "annotation-1" missing image input');
      });
      ...
      it("should validate complex workflow with multiple node types", () => {
        useWorkflowStore.setState({
          nodes: [
            ...
            createTestNode("prompt-2", "prompt", { outputText: "expanded prompt" }),
            createTestNode("generateImage-1", "generateImage", {}),
            createTestNode("annotation-1", "annotation", { sourceImage: null }),
            createTestNode("output-1", "output", {}),
          ],
          edges: [
            createTestEdge("imageInput-1", "prompt-2", "image", "image"),
            createTestEdge("prompt-1", "prompt-2", "text", "text"),
  ...
  describe("Workflow execution data flow", () => {
    ...
    describe("Text data flow through node chains", () => {
      ...
      it("should pass prompt outputText to generateImage as text input", () => {
        const llmOutput = "Generated description from LLM";

        useWorkflowStore.setState({
          nodes: [
            createTestNode("prompt-1", "prompt", { outputText: llmOutput, prompt: "" }),
            createTestNode("generateImage-1", "generateImage", {}),
          ],
          edges: [
      ...
      it("should chain prompt → generateImage correctly", () => {
        const userPrompt = "Describe this image";
        const promptOutput = "A serene landscape with rolling hills";

        useWorkflowStore.setState({
          nodes: [
            createTestNode("prompt-1", "prompt", { prompt: userPrompt, outputText: promptOutput }),
            createTestNode("generateImage-1", "generateImage", {}),
          ],
          edges: [
            createTestEdge("prompt-1", "generateImage-1", "text", "text"),
        ...
        const store = useWorkflowStore.getState();
        const inputs = store.getConnectedInputs("generateImage-1");
        expect(inputs.text).toBe(promptOutput);
      });
    });
  ...
  describe("Canvas navigation settings", () => {
    ...
    it("updateCanvasNavigationSettings persists to localStorage", () => {
      ...
      const stored = localStorage.getItem("openflows-canvas-navigation");
