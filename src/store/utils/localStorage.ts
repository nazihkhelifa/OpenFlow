...
// Storage keys
export const STORAGE_KEY = "openflows-workflow-configs";
export const COST_DATA_STORAGE_KEY = "openflows-workflow-costs";
export const GENERATE_IMAGE_DEFAULTS_KEY = "openflows-nanoBanana-defaults";
export const PROVIDER_SETTINGS_KEY = "openflows-provider-settings";
export const RECENT_MODELS_KEY = "openflows-recent-models";
...
export const getLLMDefaults = (): LLMNodeDefaults | undefined => {
  const config = loadNodeDefaults();
  const presets = config.llmPresets ?? (config.llm ? [config.llm] : []);
  const idx = config.defaultLlmPresetIndex ?? 0;
...
// Default project directory (base path for listing and creating projects)
export const getDefaultProjectDirectory = (): string => {
  if (typeof window === "undefined") return "";
  return localStorage.getItem(DEFAULT_PROJECT_DIRECTORY_KEY) || "";
};

export const setDefaultProjectDirectory = (path: string): void => {
  if (typeof window === "undefined") return;
  if (path.trim()) {
    localStorage.setItem(DEFAULT_PROJECT_DIRECTORY_KEY, path.trim());
  } else {
