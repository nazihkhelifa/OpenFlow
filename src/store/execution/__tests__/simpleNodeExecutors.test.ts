...
describe("executePrompt", () => {
  ...
  it("should resolve text and call LLM API", async () => {
    const node = makeNode("p", "prompt", { prompt: "Hello" });
    const ctx = makeCtx(node, {
      getConnectedInputs: vi.fn().mockReturnValue({
        images: [],
        videos: [],
    ...
    expect(ctx.updateNodeData).toHaveBeenCalledWith("p", expect.objectContaining({ status: "loading" }));
    expect(ctx.updateNodeData).toHaveBeenCalledWith("p", expect.objectContaining({ outputText: "generated response", status: "complete" }));
    expect(mockFetch).toHaveBeenCalledWith("/api/llm", expect.any(Object));
  });

  it("should set error when prompt is missing", async () => {
    const node = makeNode("p", "prompt", { prompt: "" });
    const ctx = makeCtx(node, {
      getConnectedInputs: vi.fn().mockReturnValue({
        images: [],
        videos: [],
    ...
    await expect(executePrompt(ctx)).rejects.toThrow("Missing prompt");
    expect(ctx.updateNodeData).toHaveBeenCalledWith("p", expect.objectContaining({ status: "error", error: expect.stringContaining("Missing prompt") }));
  });

  it("should set error on getConnectedInputs exception", async () => {
    const node = makeNode("p", "prompt", { prompt: "x" });
    const ctx = makeCtx(node, {
      getConnectedInputs: vi.fn().mockImplementation(() => {
